...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onResume() {
        ...
        asyncUpdate(getDBPlaceNames());

    }

    public ArrayList<String> getDBPlaceNames() {
        ...
        try {
            ...
            //db.customCheckDB();
            placeDB = db.openDB();
            cur = placeDB.rawQuery("select name from places order by name", null);
            Log.d("get place names", String.valueOf(cur.getColumnCount()));
            cur.moveToFirst();
            while (!cur.isAfterLast()) {

                Log.d(this.getClass().getSimpleName(), cur.getString(0));
                namesList.add(cur.getString(0));
                cur.moveToNext();
            }
